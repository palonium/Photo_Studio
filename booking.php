...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $phone = $_POST['phone'];
    $email = $_POST['email'];
    $photographerID = $_POST['photographers'];
    $photoSessionID = $_POST['photoshoots'];
    $photoStudioID = $_POST['studios'];
    $sessionTypeID = $_POST['sessionType'];
    $printRequested = isset($_POST['printing']) && $_POST['printing'] == 'yes' ? true : false;
    $bookingDate = $_POST['bookingDate'];

    // Получение user_id из сессии
    $userID = $_SESSION['user_id'];

    // Вставка данных в таблицу Contact_Information
$sql = "INSERT INTO Contact_Information (Phone_Number, Email_Address) VALUES ('$phone', '$email')";
$result = mysqli_query($conn, $sql);
$contactID = mysqli_insert_id($conn);

// Вставка данных в таблицу Client
$sql = "INSERT INTO Client (First_Name, Last_Name, Contact_ID, User_ID) VALUES ('$firstName', '$lastName', '$contactID', '$userID')";
$result = mysqli_query($conn, $sql);
$clientID = mysqli_insert_id($conn);

// Вставка данных в таблицу Booking
$sql = "INSERT INTO Booking (Client_ID, Photographer_ID, Photo_Studio_ID, Session_ID, Print_Requested, Booking_DateTime, session_type_id) VALUES ('$clientID', '$photographerID', '$photoStudioID', '$photoSessionID', '$printRequested', '$bookingDate', '$sessionTypeID')";
$result = mysqli_query($conn, $sql);
$bookingID = mysqli_insert_id($conn);

// Если печать фото запрошена, выполнить дополнительные действия
if ($printRequested) {
    $printType = $_POST['printType'];
    $sql = "UPDATE Booking SET Printing_ID = '$printType' WHERE Booking_ID = '$bookingID'";
    $result = mysqli_query($conn, $sql);
}
...
$photographersQuery = "SELECT * FROM Photographer";
$photoSessionsQuery = "SELECT * FROM Photo_Session";
$photoStudiosQuery = "SELECT * FROM Photo_Studio";
$sessionTypesQuery = "SELECT * FROM photo_session_type";
$printingTypesQuery = "SELECT * FROM Photo_Printing";

$photographersResult = mysqli_query($conn, $photographersQuery);
$photoSessionsResult = mysqli_query($conn, $photoSessionsQuery);
$photoStudiosResult = mysqli_query($conn, $photoStudiosQuery);
$sessionTypesResult = mysqli_query($conn, $sessionTypesQuery);
$printingTypesResult = mysqli_query($conn, $printingTypesQuery);
?>
...
<header class="header">
        <div class="container header__container">
            ...
            <li class="menu__item">
    <a class="menu__link dropdown" href="account.php">Аккаунт</a>
</li>
<li class="menu__item"> <a class= "menu__link" href="index.php">Назад</a></li>
        </nav>
        </div>
    </header>
    <main>
        <div class="container">
            <form class="booking-form" method="post" action="booking.php">
                ...
                <!-- Вывод данных в выпадающие списки -->
                <label for="photographers" class="form-label">Фотограф:</label>
                <select id="photographers" name="photographers" class="form-select">
                    <?php while ($row = mysqli_fetch_assoc($photographersResult)): ?>
                        <option value="<?php echo $row['Photographer_ID']; ?>"><?php echo $row['First_Name'] . ' ' . $row['Last_Name']; ?></option>
                    <?php endwhile; ?>
                </select>

                <label for="photoshoots" class="form-label">Фотосессия:</label>
                <select id="photoshoots" name="photoshoots" class="form-select">
                    <?php while ($row = mysqli_fetch_assoc($photoSessionsResult)): ?>
                        <option value="<?php echo $row['Session_ID']; ?>"><?php echo $row['Name']; ?></option>
                    <?php endwhile; ?>
                </select>

                <label for="studios" class="form-label">Фотозал:</label>
                <select id="studios" name="studios" class="form-select">
                    <?php while ($row = mysqli_fetch_assoc($photoStudiosResult)): ?>
                        <option value="<?php echo $row['Photo_Studio_ID']; ?>"><?php echo $row['Name']; ?></option>
                    <?php endwhile; ?>
                </select>

                <label for="sessionType" class="form-label">Тип фотосессии:</label>
                <select id="sessionType" name="sessionType" class="form-select">
                    <?php while ($row = mysqli_fetch_assoc($sessionTypesResult)): ?>
                        <option value="<?php echo $row['id']; ?>"><?php echo $row['name']; ?></option>
                    <?php endwhile; ?>
                </select>

                <label for="printing" class="form-label">Требуется печать:</label>
                <select id="printing" name="printing" class="form-select">
                    <option value="yes">Да</option>
                    <option value="no">Нет</option>
                </select>

                <!-- Вывод данных в выпадающий список типов печати -->
                <label for="printType" class="form-label">Тип печати:</label>
                <select id="printType" name="printType" class="form-select">
                    <?php while ($row = mysqli_fetch_assoc($printingTypesResult)): ?>
                        <option value="<?php echo $row['Printing_ID']; ?>"><?php echo $row['Print_Type']; ?></option>
                    <?php endwhile; ?>
                </select>

                <label for="bookingDate" class="form-label">Дата заказа:</label>
                <input type="datetime-local" id="bookingDate" name="bookingDate" class="bookdate form-input" required>
...
mysqli_free_result($photographersResult);
mysqli_free_result($photoSessionsResult);
mysqli_free_result($photoStudiosResult);
mysqli_free_result($sessionTypesResult);
mysqli_free_result($printingTypesResult);
