...
<body>
    <header class="header">
        ...
        <a href="#">
            ...
            <nav class="header__menu">
                ...
                <li class="menu__item"> <a class= "menu__link" href="#photografers">Фотографы</a></li>
                <li class="menu__item"> <a class= "menu__link" href="#photoshoots">Фотосессии</a></li>
                <li class="menu__item"> <a class= "menu__link" href="#booking">Запись</a></li>
                <li class="menu__item"> <a class= "menu__link" href="#tax">Тариф</a></li>
                <li class="menu__item"> <a class= "menu__link dropdown" href="account.php"> Аккаунт
                <li class="menu__item"> <a class= "menu__link" href="#"> </a>
    ...
    <main>
        <section class="main">
            <div class="container">
                <h1 class="main__title ">Фотостудия <span class="main__title-span">"Lumos"</span> — арт-пространство площадью 2 600 квадратных метров, состоящее из 27 съемочных локаций</h1>
                <article class="main__content">
                    <p class="main__text text">Все залы фотостудии выполнены в различных интерьерных концепциях и стилистиках с использованием многообразия предметов интерьера.
                    ...
                    <img src="images/main.webp" alt="" class="main__img">
                </article>
            </div>
        </section>
        <div class="container box " id="photografers">
            <h2 class="title">Наши фотографы</h2>
        <div id="photographerList" class="photographers-wrapper">
            <?php
            include 'config.php';
            ...
            if ($result !== false && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<div class='photographer-photo-wrapper'>";
                    echo "<img src='" . $row["PhotoPath"] . "' alt='Photographer Photo' class='photographer-photo'>";
                    echo "</div>";
        ...
        <div class="container box" id="rooms">
        <h2 class="title">Наши фотозалы</h2>
        <div id="photoStudioList" class="photographers-wrapper">
            <?php
            include 'config.php';

            $sql = "SELECT * FROM Photo_Studio ps
                    INNER JOIN Photo_Studio_Location loc ON ps.Location_ID = loc.Location_ID";
            $result = $conn->query($sql);

            if ($result !== false && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<div class='photo-studio'>";
                    echo "<div class='studio-photo'>";
                    echo "<img src='" . $row["Photo"] . "' alt='Studio Photo'>";
                    echo "</div>";
                    echo "<div class='studio-info'>";
                    echo "<p class='studio-name title-small'>" . $row["Name"] . "</p>";
                    echo "<p class='studio-description'>" . $row["Description"] . "</p>";
                    echo "<p class='studio-address title-small'>" . $row["Street"] . ", " . $row["House"] . "</p>";
                    echo "</div>";
                    echo "</div>";
                }
            } else {
    ...
    <div class="container box" id="photoshoots">
    <h2 class="title">Наши фотосессии</h2>
        <div id="photoSessionList" class="sessions-wrapper">
            <?php
            include 'config.php';

            $sessionSql = "SELECT * FROM Photo_Session";
            $sessionResult = $conn->query($sessionSql);

            if ($sessionResult !== false && $sessionResult->num_rows > 0) {
                while ($sessionRow = $sessionResult->fetch_assoc()) {
                    echo "<div class='photo-session'>";
                    echo "<div class='session-photo'>";
                    // Предположим, что у вас есть соответствующее поле с фото фотосессии
                    echo "<img src='" . $sessionRow["Photo"] . "' alt='Session Photo'>";
                    echo "</div>";
                    echo "<div class='session-info'>";
                    echo "<p class='session-name title-small'>" . $sessionRow["Name"] . "</p>";
                    echo "<p class='session-description'>" . $sessionRow["Description"] . "</p>";
    ...
    <div class="container container-types box" id="tax">
    <h2 class="title title-types">Тарифы</h2>
        <div id="sessionTypeList" class="session-types-wrapper">
            <?php
            include 'config.php';

            $typeSql = "SELECT * FROM photo_session_type";
            $typeResult = $conn->query($typeSql);

            if ($typeResult !== false && $typeResult->num_rows > 0) {
                while ($typeRow = $typeResult->fetch_assoc()) {
                    echo "<div class='session-type'>";
                    echo "<div class='session-type-info'>";
                    echo "<p class='title-small'>" . $typeRow["name"] . "</p>";
                    echo "<p>Duration: " . $typeRow["duration"] . " hours</p>";
                    echo "<p>Cost: $" . $typeRow["cost"] . "</p>";
                    echo "</div>";
                    echo "</div>";
                }
            } else {
        ...
        <div class="background__container" id="booking">
                <div class="main-booking__container content">
                    <h2 class="main-booking__title">Скорее записывайтесь на фотосессию в Lumos</h2>
                    <button class="main-booking__button" onclick="window.location.href='booking.php'">Записаться</button>
                </div>
        </div>
    </main>
    ...
    window.onload = function () {
        ...
        xmlhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("photographerList").innerHTML = this.responseText;
            }
        };
        xmlhttp.open("GET", "getPhotographers.php", true);
        xmlhttp.send();
    };
</script>
