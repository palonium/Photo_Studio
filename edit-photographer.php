...
// Обработка добавления нового фотографа
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    // Добавление фотографа и полного пути к фотографии в базу данных
    $sql = "INSERT INTO Photographer (First_Name, Last_Name, Experience, PhotoPath) VALUES ('$firstName', '$lastName', $experience, '$targetFile')";
...
if (isset($_GET['delete'])) {
    ...
    $sql = "SELECT PhotoPath FROM Photographer WHERE Photographer_ID = $id";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $filePath = $row['PhotoPath'];
    unlink($filePath);

    // Удаление информации о фотографе из базы данных
    $sql = "DELETE FROM Photographer WHERE Photographer_ID = $id";
    $result = $conn->query($sql);
}
// Обработка редактирования фотографа
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit'])) {
    ...
    $sql = "UPDATE Photographer SET First_Name='$firstName', Last_Name='$lastName', Experience=$experience WHERE Photographer_ID=$id";
    $result = $conn->query($sql);
}
...
<div class="container">
<h1>Photographers</h1>
    <!-- Форма для добавления нового фотографа -->
<form method="post" action="" enctype="multipart/form-data" class="form-container">
        ...
        <button type="submit" name="add">Add Photographer</button>
    </form>

    <!-- Вывод списка фотографов -->
    <table border="1" class="photographer-table">
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    <td class='table-cell'>{$row['Photographer_ID']}</td>
                    <td class='table-cell'>{$row['First_Name']}</td>
                    <td class='table-cell'>{$row['Last_Name']}</td>
                    <td class='table-cell'>{$row['Experience']}</td>
                    <td class='table-cell'>
                        <a class='delete-button'href='?delete={$row['Photographer_ID']}'>Delete</a>
                        <button onclick='editPhotographer({$row['Photographer_ID']}, \"{$row['First_Name']}\", \"{$row['Last_Name']}\", {$row['Experience']})' class='form-button'>Edit</button>
                    </td>
                </tr>";
        }
        ?>
    ...
    <!-- Форма для редактирования фотографа (скрыта по умолчанию) -->
    <form method="post" action="" id="editForm" style="display: none;" class="form-container">
        <input type="hidden" name="photographer_id" id="editId" class="form-input">
        ...
        <input type="number" name="experience" id="edit_experience" class="form-input" required>
        <button type="submit" name="edit" class="form-button">Save Changes</button>
        <button type="button" onclick="cancelEdit()" class="form-button">Cancel</button>
    </form>

</div>

<!-- JavaScript для управления видимостью формы редактирования и передачи данных при редактировании -->
<script>
    function editPhotographer(id, firstName, lastName, experience) {
