...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["add_photo_session"])) {
        $name = $_POST["name"];
        $description = $_POST["description"];
        $photo = uploadPhoto();

        $sql = "INSERT INTO Photo_Session (Name, Description, Photo) VALUES ('$name', '$description', '$photo')";
        $conn->query($sql);
    }

    // Изменение фотосессии
if (isset($_POST["edit_photo_session"])) {
    $sessionID = $_POST["session_id"];
    
    // Извлечение данных из базы данных для отображения в форме
    $sql = "SELECT * FROM Photo_Session WHERE Session_ID=$sessionID";
    ...
    if ($result->num_rows > 0) {
        ...
        echo "Photo Session not found!";
        exit;
    }

    // Переносим обработку фотографии в саму функцию
    $photo = uploadPhoto($photo);

    // Замените $_POST на извлеченные значения
    $name = isset($_POST["name"]) ? $_POST["name"] : $name;
    $description = isset($_POST["description"]) ? $_POST["description"] : $description;

    $sql = "UPDATE Photo_Session SET Name='$name', Description='$description', Photo='$photo' WHERE Session_ID=$sessionID";
    $conn->query($sql);
}
    ...
    // Удаление фотосессии
    if (isset($_POST["delete_photo_session"])) {
        $sessionID = $_POST["session_id"];

        $sql = "DELETE FROM Photo_Session WHERE Session_ID=$sessionID";
        $conn->query($sql);
    }
}

// Функция для загрузки фотографии
function uploadPhoto($existingPhoto) {
    $targetDir = "uploads/";

    // Если фото было загружено
    ...
    // Если фото не было загружено, возвращаем существующее фото
...
<head>
    ...
    <title>Edit Photo Sessions</title>
</head>
<body>
<header class="header">
        ...
        <a href="#">
            ...
            <nav class="header__menu">
                <ul class="menu">
                <li class="menu__item"> <a class= "menu__link" href="admin.php">Назад</a></li>                
            </ul></nav>
        </div>
    </header>
<div class="container">
        <!-- Форма для добавления фотосессии -->
        <form action="" method="post" enctype="multipart/form-data" class="form-container">
            <h2>Add Photo Session</h2>
            ...
            <br>
            <input type="submit" name="add_photo_session" value="Add Photo Session" class="form-button">
        </form>

        <!-- Таблица существующих фотосессий -->
        <h2>Existing Photo Sessions</h2>
        <table border="1" class="photographer-table">
            <tr>
    ...
    <?php foreach ($photoSessions as $session): ?>
        <tr>
            <td class="table-cell"><?= $session["Session_ID"] ?></td>
            <td class="table-cell"><?= $session["Name"] ?></td>
            <td class="table-cell"><?= $session["Description"] ?></td>
            <td class="table-cell"> <div class="photo-wrapper"><img src="<?= $session["Photo"] ?>" alt="Photo"></div></td>
            <td class="table-cell">
                <button class='form-button' onclick="showEditForm(<?= $session["Session_ID"] ?>, '<?= $session["Name"] ?>', '<?= $session["Description"] ?>')">Edit</button>
            </td>
            <td class="table-cell">
                <form action="" method="post">
                    <input type="hidden" name="session_id" value="<?= $session["Session_ID"] ?>">
                    <input class='delete-button' type="submit" name="delete_photo_session" value="Delete">
                </form>
            </td>
        </tr>
    <?php endforeach; ?>
...
<!-- Форма для изменения фотосессии (изначально скрыта) -->
<div id="editForm" class="edit-modal">
    <h2>Edit Photo Session</h2>
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <br>
        <input type="submit" name="edit_photo_session" value="Edit Photo Session" class="form-button">
        <!-- Кнопка закрытия модального окна -->
        <button type="button" onclick="closeEditForm()">Close</button>
    </form>

    <script>
    function showEditForm(sessionID, name, description) {
