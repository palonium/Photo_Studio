...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["edit_user"])) {
        ...
        // Переносим обработку пароля в саму функцию
        ...
        $sql = "UPDATE User SET Login='$login', Role='$role'$passwordUpdate WHERE User_ID=$userID";
        $conn->query($sql);
    }
...
<form class="form-container" action="" method="post">
        ...
        <br>
        <input class="form-button" type="submit" name="add_user" value="Add User">
 </form>


 <h2 class="table-header">Existing Users</h2>
    <table class="photographer-table" border="1">
        ...
        <?php foreach ($users as $user): ?>
            <tr>
                ...
                <td class="table-cell">
                    <button class="form-button" onclick="showEditForm(<?= $user["User_ID"] ?>, '<?= $user["Login"] ?>', '<?= $user["Role"] ?>')">Edit</button>
                </td>
                <td class="table-cell">
                    <form action="" method="post">
                        <input type="hidden" name="user_id" value="<?= $user["User_ID"] ?>">
                        <input class="delete-button" type="submit" name="delete_user" value="Delete">
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
 ...
 <!-- Форма для изменения пользователя (изначально скрыта) -->
    <div id="editForm" style="display: none;">
        <h2>Edit User</h2>
        <form action="" method="post">
            <input type="hidden" id="user_id" name="user_id">
            <label class="form-label" for="login">Login:</label>
            <input class="form-input" type="text" id="login" name="login" required>
            <br>
            <label class="form-label" for="password">Password:</label>
            ...
            <br>
            <input class="form-button" type="submit" name="edit_user" value="Edit User">
        </form>
    </div>
</div>
    ...
    <!-- Добавленный скрипт JavaScript -->
    <script>
        function showEditForm(userID, login, role) {
