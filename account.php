...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Если форма была отправлена, обработайте данные
    ...
    if ($conn->query($sql) === TRUE) {
        echo "Аккаунт успешно обновлен.";
    } else {
        echo "Ошибка при обновлении аккаунта: " . $conn->error;
    }
}

// Здесь выводите форму для редактирования аккаунта
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<header class="header">
    ...
    <div class="container-new">
        <h2 class="heading">Редактирование аккаунта для пользователя <?php echo $_SESSION['login']; ?></h2>

        <form action="account.php" method="post" class="form">
            <label for="new_password" class="label">Новый пароль:</label>
            <input type="password" name="new_password" id="new_password" required class="input">

            <input type="submit" value="Обновить аккаунт" class="submit-btn">
        </form>
    </div>
</body>
</html>
